...
function generateGame(lastTwoColor) {
    ...
    // 理论依据：合4倍数 / 双偶数 / 双零效应
         ...
         if (Math.random() < 0.6 && simulatedHistory.length > 0) {
            ...
            if (last.includes('闲对')) actual = '闲对';
            else if (last.includes('庄对')) actual = '庄对';
            else actual = '对子'; // 默认
        } else {
            // 随机分配
            actual = (Math.random() < 0.5) ? '闲对' : '庄对';
        }
    }
    // 4. 闲7 (Priority 4)
    ...
    // 更新模拟历史
    if (actual.includes('对') || actual === '平牌' || actual === '庄6' || actual === '闲7') {
         // 只有高赔事件才可能进入“有效历史”供遗传参考？
         // 逻辑中遗传只参考“闲对/庄对”。
