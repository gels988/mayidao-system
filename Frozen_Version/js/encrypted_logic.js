    ...
    function ModelA(c){
        ...
        if(P===8){
            // Panda Logic (Updated)
             ...
             // 3. Interval 1 Echo: Gap of 1 round from previous Panda (Index -2)
             // 4. "Only 1 time" -> Prevent infinite echo chain.
             //    If T-2 was Panda, check T-4. If T-4 was also Panda, assume T-2 was an Echo (or frequent), so stop.
             
             var lcStr = (typeof lc === 'string') ? lc : '';
             var isBlueTrend = lcStr.endsWith('蓝');
             
             var gapPanda = false;
             if (h.length >= 2 && h[h.length-2] === '熊猫') {
                 // Check T-4 to prevent chain: P -> N -> P -> N -> P...
                 // If T-4 was also Panda, we suppress this 2nd Echo.
                 if (h.length < 4 || h[h.length-4] !== '熊猫') {
                     gapPanda = true;
                 }
             }
             
             if (isBlueTrend || gapPanda) {
                 res = '熊猫';
             }
        }
