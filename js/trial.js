...
const TRIAL_DAYS = 10;
const CONSUME_RATE_FREE = 20;  // 免费用户消耗
const CONSUME_RATE_ACTIVATED = 5; // 激活后消耗
...
class TrialManager {
    ...
    // 扣费（根据激活状态决定费率）
    deductForPrediction() {
        this.checkActivationToken(); // 每次扣费前检查激活
    ...
    getStatusMessage() {
        if (this.state.isActivated) {
            return `✅ ACTIVATED | Balance: ${this.state.balance} G`;
        }
        ...
        return `⏳ TRIAL: ${days} Days Left | Bal: ${this.state.balance} G (-20G/Turn)`;
    }
}
